                    ...
                    <form method="POST" action="checkAvailability.php" class="mb-5">
                        <div class="row">
                            <div class="col-md-2">
                                ...
                                <input type="date" name="check_in" id="check_in" value="<?= htmlspecialchars($checkin_date ?? '') ?>" class="form-control" required>
                            </div>
                            <div class="col-md-2">
                                <label>Check-out Date:</label>
                                <input type="date" name="check_out" id="check_out" value="<?= htmlspecialchars($checkout_date ?? '') ?>" class="form-control" required>
                            </div>
                            <div class="col-md-2">
                                <label>No. of Rooms:</label>
                                <input type="number" name="num_rooms" min="1" value="1" class="form-control" required>
                            </div>
                            <div class="col-md-2">
                                <label>No. of Adults:</label>
                                <input type="number" name="guests" min="1" value="2" class="form-control" required>
                            </div>
                            <div class="col-md-2">
                                <label>No. of Children:</label>
                                <input type="number" name="num_children" min="0" value="0" class="form-control">
                            </div>
                ...
                <div class="col-xl-4 col-md-4">
                    <div class="single_offers">
                        ...
                        <?php if ($room['available_qty'] > 0 || is_null($room['available_qty'])): ?>
                            ...
                            <a href="bookingForm.php?room_id=<?= $room['id'] ?>&check_in=<?= urlencode($_GET['check_in'] ?? '') ?>&check_out=<?= urlencode($_GET['check_out'] ?? '') ?>&no_of_rooms=<?= $_GET['no_of_rooms'] ?? 1 ?>&guests=<?= $_GET['guests'] ?? 2 ?>&children=<?= $_GET['children'] ?? 0 ?>" class="btn btn-primary">
    Book Now
</a>
